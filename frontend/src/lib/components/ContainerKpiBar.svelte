...
<script lang="ts">
	...
	let workspaceSizeGb = $derived(asNumber(workspace?.sizeGb) || (workspaceGbLimit ?? 0));
	...
	let hasWorkspaceMetric = $derived(
		workspaceSizeGb > 0 || workspaceUsedGb > 0 || Boolean(workspace?.device) || Boolean(workspaceGbLimit),
	);
