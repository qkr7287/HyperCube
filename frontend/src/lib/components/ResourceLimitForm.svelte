<script lang="ts">
	...
	let workspaceMax = $derived(Math.max(10, hostWorkspacePoolGb ?? 0, recommendedWorkspaceGb, minWorkspaceGb, workspaceGb));
	...
	let workspaceHostShare = $derived(
		hostWorkspacePoolGb ? Math.round((workspaceGb / hostWorkspacePoolGb) * 100) : null,
	);
	...
	<div class="limit-grid">
		...
		<div class="limit-row" data-warning={workspaceGb < minWorkspaceGb}>
			...
			<div class="limit-meta">
				...
				<span>
					{#if workspaceHostShare === null}
						quota pool 정보 없음
					{:else}
						host의 {workspaceHostShare}% 점유{hostWorkspaceHardEnforcement ? ' · hard enforced' : ' · soft only'}
					{/if}
				</span>
			</div>
