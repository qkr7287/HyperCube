...
class MonitoringConsumer(AsyncWebsocketConsumer):
    ...
    def _update_container_workspace_from_create_result(self, data: dict) -> None:
        ...
        update_fields = []
        device = workspace.get("device") or workspace.get("workspaceDevice")
        if device:
            container.workspace_device = str(device)[:120]
            update_fields.append("workspace_device")
        size_gb = _positive_int_or_none(workspace.get("sizeGb") or workspace.get("size_gb"))
    ...
    def _handle_capacity_report(self, data: dict) -> None:
        ...
        lvm = disk.get("lvm") if isinstance(disk.get("lvm"), dict) else {}
        ...
        if lvm.get("available") is False:
            updates["lvm_pool_size_gb"] = None
        else:
            _set_if_int(updates, "lvm_pool_size_gb", lvm.get("thinPoolSizeGb"))
...
def _container_limit_defaults_from_request(req, updated_at) -> dict:
    defaults = {
        ...
        "workspace_gb_limit": req.workspace_gb,
    }
