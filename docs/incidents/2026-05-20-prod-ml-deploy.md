...
2. **🔶 agent → backend prepare command_response 라우팅** — backend 코드 점검
   ...
   (2026-05-20) backend 에 **self-heal** 추가 — `ModelPrepareJob.last_progress_at`
   watchdog (`MODEL_PREPARE_PROGRESS_TIMEOUT_SECONDS`, 기본 900s) 으로 stuck 감지
   → `requeue_prepare_job` 재dispatch (`MODEL_PREPARE_MAX_ATTEMPTS` 초과 시 FAILED),
   agent reconnect edge 에서 `reconcile_agent_prepare_jobs` 즉시 복구. 운영자
   수동 READY 마크 불필요. **남은 agent 작업**: `prepare_model_assets` 멱등성
   (sha256 일치 시 skip) 보장, `query_model_cache` command 로 "파일 있는데 응답
   유실" 케이스 무손실 복구.
3. **🔶 prepare progress bytes_done 미누적** — backend `handle_prepare_progress`
   는 `bytesDone`/`bytes_done` 둘 다 수용 — 계약서와 일치, backend 버그 없음.
   agent 측 emit 키 확인 필요. watchdog(2번)이 bytes_done=0 이어도 stuck 을
   잡으므로 운영 리스크는 해소.
