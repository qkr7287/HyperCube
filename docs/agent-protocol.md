...
`ram_total_mb`, `disk_total_gb`, `lvm_pool_size_gb`, `nic_speed_mbps`,
`filesystem`, `capacity_updated_at`을 갱신한다. 전체 schema는
...
`workspace.sizeGb` and `sharedMounts` are omitted when the target Agent has no
LVM thin capacity. The backend accepts both legacy `command_response` and the
agent sprint message `create_container_result`. Include `requestId` whenever
possible; if it is absent, backend only updates an already-known `Container` by
`containerId` and `data.workspace.device`.
