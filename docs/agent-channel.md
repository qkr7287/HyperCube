...
| [#14](https://github.com/qkr7287/HyperCube-agent/issues/14) | prepare_model_assets — backend_stream payload sha256 read fix | 발행 2026-05-13. GPU/ML Workspace v1 차단 마지막 한 칸. handler가 `params.assets[].sha256` 을 못 읽어 1ms 내 거부. core test/seed/UI 모두 완성, agent 측 1건 fix 만 남음. |
| [#15](https://github.com/qkr7287/HyperCube-agent/issues/15) | container_metrics — per-container GPU block (usage / memoryUsed / memoryTotal) | 발행 2026-05-14. HyperCube `b872374` 에서 GPU 코어/VRAM 컬럼·KPI·차트 분리 완료, agent 가 `data.gpu` 미전송이라 모든 값 NULL. `nvidia-smi -i <device_id>` 1회 호출로 컨테이너별 grouping 합산 push. MIG/FULL 독점 환경 가정. |
