...
Audited evidence chain includes:
...
| PR 1: `capacity_report` handling | `backend/apps/common/consumers.py` updates Agent capacity fields; `backend/apps/common/tests/test_consumers.py` covers full and partial/null reports | Done |
...
| PR 3: create result workspace persistence | `backend/apps/common/consumers.py` and `services/workspace.py` store `workspace.device` and `sizeGb`; consumer tests cover `create_container_result` alias and no-request-id fallback | Done |
...
| Commit and push | Remote `dev` evidence chain verified through `7f2ca42168e4ed1219f3478c5b425298de00fa96`; GitHub compare reported `dev` identical to that commit | Done |
